...
app.get('/api/statystyki', (req, res) => {
  ...
  const kpi = db.prepare(`
    SELECT COUNT(*) as liczba, ROUND(AVG(cena), 0) as srednia_cena, ROUND(AVG(cena_m2), 0) as srednia_cena_m2
    FROM oferty ${where}
  `).get(params);

  const cenyBins = db.prepare(`
    SELECT ROUND(cena / 100000) * 100000 as bin, COUNT(*) as ile
    FROM oferty ${where} AND cena IS NOT NULL
    GROUP BY bin ORDER BY bin
  `).all(params);

  const metrazBins = db.prepare(`
    SELECT ROUND(metraz / 10) * 10 as bin, COUNT(*) as ile
